...
def encoder(features, vocab_input_size, embedding_dim, units, batch_size, dropout_rate):
    ...
    # TODO: change multi-lstm ceil, done
    ...
    stacked_lstm_cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.DropoutWrapper
                                                     (tf.nn.rnn_cell.BasicLSTMCell(units),
                                                      output_keep_prob=1-dropout_rate) for _ in range(4)])
...
def model_fn(features, labels, mode, params):
    train_target_tokenizer = params['train_target_tokenizer']
    vocab_input_size = params['vocab_input_size']
    ...
    source_output, source_state = encoder(features, vocab_input_size, embedding_dim, units,
                                          batch_size, dropout_rate)
    dec_input = tf.expand_dims([train_target_tokenizer.word_index['<s>']] * batch_size, 1)

    if mode == tf.estimator.ModeKeys.TRAIN:
        for idx in range(1, labels.shape[1]):
            ...
            mask = tf.math.logical_not(tf.math.equal(labels[:, idx], 0))
            loss = tf.losses.SparseCategoricalCrossentropy(labels[:, idx], prediction,
