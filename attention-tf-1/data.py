...
def sort_data_files(source_data_path, target_data_path):
    ...
    indices = sorted(range(len(words)), key=lambda k: words[k])
...
def batch_shuffle(source_data_path, target_data_path, batch_size):
    ...
    source_batches = [source[i:i+batch_size] for i in range(0, len(source), batch_size)]
    target_batches = [target[i:i+batch_size] for i in range(0, len(target), batch_size)]
    indices = [i for i in range(len(source_batches))]
    shuffle(indices)
