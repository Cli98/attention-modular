...
def train():
    ...
    initializer = tf.random_uniform([vocab_input_size, EMBED_DIM], -0.1, 0.1)
    ...
    features = {'x':train_source_tensor, 'y':train_target_tensor}
    nmt_estimator = tf.estimator.Estimator(model_fn=model_fn,
                                            model_dir=os.path.join(".", 'nmt'),
                                            params=params)
    input_fn_train = train_input_fn(train_source_tensor, train_target_tensor, buffer_size, BATCH_SIZE)
